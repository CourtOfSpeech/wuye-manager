...
<el-input v-model="searchMap.state" placeholder="是否可见 1：可见 2：不可见"></el-input></el-form-item>
  ...
  <el-table
    ...
    style="width: 100%">
          ...
          <el-table-column prop="nickname" label="发布人昵称" width="80"></el-table-column>
          ...
          <el-table-column prop="thumbup" label="点赞" width="80"></el-table-column>
          <el-table-column prop="vistis" label="访问量" width="80"></el-table-column>
...
<script>
import spitslotApi from '@/api/spitslot'
export default {
  data() {
    return {
      list: [],
  ...
  methods: {
    fetchData() {
      spitslotApi.search(this.currentPage, this.pageSize, this.searchMap).then(response => {
        this.list = response.data.rows
        this.total = response.data.total
      })
    },
    handleSave() {
      spitslotApi.update(this.id, this.pojo).then(response => {
        this.$message({
    ...
    handleEdit(id) {
      ...
      if (id !== '') { // 修改
        spitslotApi.findById(id).then(response => {
          if (response.flag) {
    ...
    handleDelete(id) {
      this.$confirm('确定要删除此纪录吗?', '提示', {
        ...
        spitslotApi.deleteById(id).then(response => {
